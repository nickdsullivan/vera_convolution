...
class ConvLoRA(nn.Module, LoRALayer):
    def __init__(self, conv_module, in_channels, out_channels, kernel_size, r=0, padding = 0, stride = 1, lora_alpha=1, lora_dropout=0., merge_weights=True, use_vera=True, **kwargs):
        super(ConvLoRA, self).__init__()
        self.conv = conv_module(in_channels, out_channels, padding=padding, stride = stride, kernel_size=kernel_size, **kwargs)
        self.is_vera=use_vera
        LoRALayer.__init__(self, r=r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, merge_weights=merge_weights)
        assert isinstance(kernel_size, int)
        # Actual trainable parameters
        if r > 0:
            if self.is_vera == True:
                self.lora_A = nn.Parameter(
